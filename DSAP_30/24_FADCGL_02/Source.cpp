...
void findCombinations(int A[], vector<int> &outputCombVec, int currentIdx, int lenCombinations)
{
	...
	while (currentIdx > 0 && A[currentIdx] == A[currentIdx - 1])
	{
		currentIdx--;
	}
	outputCombVec.push_back(A[currentIdx]);
	findCombinations(A, outputCombVec, currentIdx - 1, lenCombinations - 1);

	outputCombVec.pop_back();
	findCombinations(A, outputCombVec, currentIdx - 1, lenCombinations);
}
int main()
{
	int A[] = { 1, 1, 2, 2, 2, 3, 3 };
	...
	sort(A, A + lenArr);
	findCombinations(A, outputCombVec, lenArr -1, lenCombinations);
}
